<?php
  ...
  list($root, $data) = $Gresponse->GetParsedXML($xml_response);
	//print_r($Gresponse->GetParsedXML($xml_response));
				...
				$datagoogle=$databaseObj->sqlinsert("select * FROM  orders WHERE  serial_number='".$data[$root]['serial-number']."'");
				 ...
				 if($number_of_record==0)	
				{
						$databaseObj->sqlinsert("insert into orders set 
											...
											serial_number							='".$paymentData['serial-number']."',
						...
						if(isset($paymentData['shopping-cart']['item-name']))
						{
							...
							foreach ($paymentData['shopping-cart'] as $key=>$valuecart){
												$databaseObj->sqlinsert("insert into order_items set 
														order_id 			='".$last_order_id."',
														sell_item_id 	='".$valuecart['merchant-item-id']['VALUE']."',
														item_amount 	='".$valuecart['unit-price']['VALUE']."',
														quantity 			='".$valuecart['quantity']['VALUE']."'
												");
							}
