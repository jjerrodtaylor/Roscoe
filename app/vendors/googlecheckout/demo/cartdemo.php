<?php
  ...
  include(__ROOT__.'/library/googlecart.php'); 
	include(__ROOT__.'/library/googleitem.php'); 
	include(__ROOT__.'/library/googleshipping.php'); 
	include(__ROOT__.'/library/googletax.php'); 
 class  GoogleCheckoot1{
  
		function Usecase($data,$dicount_shipping) {
      ...
      $cart->SetMerchantPrivateData('buyer_user_id-'.$data[0]['sell_item_buyler_id'].'-seller_user_id-'.$data[0]['seller_user_id']);
     /*  if($total_count < 3){
             $ship_1 = new GoogleFlatRateShipping("USPS Priority Mail", 0);
      }else{
             $ship_1 = new GoogleFlatRateShipping("USPS Priority Mail", 0);
      } */
			$ship_1 = new GoogleFlatRateShipping("USPS Priority Mail", $dicount_shipping);
      ...
      $cart->SetEditCartUrl("http://64.15.136.251:8080/cellsolo/orders/cart/");
      // Specify "Return to xyz" link
      $cart->SetContinueShoppingUrl("http://64.15.136.251:8080/cellsolo/orders/successful");
...
// Checkout and redirect the buyer to the corresponding page
  ...
  function CarrierCalcUsecase() {
    ...
    $merchant_id = "674074763997627";  // Your Merchant ID
    $merchant_key = "IdWCl8RSltQgl6NZe0nP1A";  // Your Merchant Key
    $server_type = "sandbox";
    $currency = "USD";
    $cart = new GoogleCart($merchant_id, $merchant_key, $server_type, $currency); 
