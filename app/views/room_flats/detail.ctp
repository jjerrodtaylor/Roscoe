	...
	<div class="InnerMidCntR">
		...
		<h2 class="InnerMidCntRHd">Room/Flat Detail	<span class="back"><?php e($html->link('Back',array('controller'=>'room_flats','action'=>'search','page:'.$page,'#middle'))); ?></span></h2>
			<?php			
			if(count($data)>0){?>
				<ul class="MyAccCnts RoomFlateDe">
					<li class="RoomFlateLeft">
						<div class="home_head">
							...
							<p>
								<em>
									<?php e(ucfirst($data['RoomFlat']['street_address']) . "," . ucfirst($data['RoomFlat']['city_name']) . "," . ucfirst($data['State']['name']) . "-" . $data['RoomFlat']['zipcode'].",".$data['Country']['iso_code']); ?>
								</em>
							</p>
						</div>
						<div class="home_left">
